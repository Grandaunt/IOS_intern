...
class ZJCitySelectController: UIViewController
{
    ...
    override func viewDidLoad()
    {
        super.viewDidLoad()
        self.initNav()
        self.dealData(array: ZJCitysData.shard.citys)
    }
...
//    {
//        BaseViewModel().post(url: kGetAllCitysURL, param: nil, MBProgressHUD: true, success: { (resp) in
//            let array = resp?["list"].arrayObject
//            let dataArray = ZJCityModel.mj_objectArray(withKeyValuesArray: array) as! [ZJCityModel]
//            self.dealData(array: dataArray)
//        }, noData: nil, failure: nil)
//    }
    
    fileprivate func initNav()
    {
        let rightItem = UIBarButtonItem(title: "取消", style: .plain, target: self, action: #selector(back))
...
extension ZJCitySelectController:UITableViewDelegate,UITableViewDataSource
{
