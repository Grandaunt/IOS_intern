...
//         获取列(menu经过reload以后会把controls数组清空，会出现col取不到的现象，建议最好controlers里面的控制器不要共用)
//        NSInteger col = [self.controllers indexOfObject:note.object];
        
        int col = 0;
        for (int i = 0; i < self.controllers.count; i ++ )
        {
            if ([self.controllers[i] class] == [note.object class])
            {
                col = i;
            }
        }
...
- (void)clear
{
#pragma mark - 这里不能置nil，置nil以后返回该界面就不是同一个contentView了，所以dismiss方法失效
//    self.bottomLine = nil;
//    self.coverView = nil;
