    ...
    if ([super initWithFrame:frame]) {
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            if (i == imageNameArray.count-1 && isHidden == NO) {
                ...
                UIButton *startButton = [[UIButton alloc]initWithFrame:CGRectMake(DDScreenW*0.3, DDScreenH*0.86, DDScreenW*0.4, DDScreenH*0.06)];
                [startButton setTitle:@"立即体验" forState:UIControlStateNormal];
                [startButton setTitleColor:[UIColor colorWithRed:64/255.0 green:175/255.0 blue:255/255.0 alpha:1.0] forState:UIControlStateNormal];
                startButton.layer.masksToBounds = true;
                startButton.layer.cornerRadius = DDScreenH*0.06*0.5;
                startButton.layer.borderColor = [UIColor colorWithRed:64/255.0 green:175/255.0 blue:255/255.0 alpha:1.0].CGColor;
                startButton.layer.borderWidth = 0.5;
                [startButton.titleLabel setFont:[UIFont systemFontOfSize:17]];
//                [startButton setBackgroundImage:[UIImage imageNamed:@"GuideImage.bundle/guideImage_button_backgound"] forState:UIControlStateNormal];
        ...
        self.imagePageControl.currentPageIndicatorTintColor = [UIColor colorWithRed:64/255.0 green:175/255.0 blue:255/255.0 alpha:1.0];
...
- (void)buttonClick:(UIButton *)button {
    ...
    if ([self.delegate respondsToSelector:@selector(DHGuidePageHUDHideAction:)])
    {
        [self.delegate DHGuidePageHUDHideAction:self];
    }
    else
    {
...
- (void)removeGuidePageHUD {
    ...
    if ([self.delegate respondsToSelector:@selector(DHGuidePageHUDWillHide:)])
    {
        [self.delegate DHGuidePageHUDWillHide:self];
    }
